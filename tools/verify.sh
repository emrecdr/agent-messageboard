...
# `release.yml` is generated by `dist`, whose default output uses floating tags; the pin lives in
# `dist-workspace.toml` and a regeneration without it would unpin every action in silence.
run "tools/check_action_pins.py" python3 tools/check_action_pins.py
# `cargo` cannot see an unused dependency — nothing is compiled from it, so there is no item to
# lint. `anyhow` survived the life of the project behind a comment saying where it was used.
run "tools/check_unused_deps.py" python3 tools/check_unused_deps.py
