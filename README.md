...
**Status: built and working.** 744 tests (746 on Linux), including multi-process concurrency and hook-safety
...
| `amb inbox [--unread] [--from A] [--kind K] [--limit N] [WORDS…]` | What is waiting for you. **A list: the newest 25, each body previewed** — it says how many it kept back and `--limit 0` lists all of them (D137). `amb read <id>` returns one whole. The header counts everything selected, not the window; `*` marks unread, and `--json` rows carry `"read"`. Filters narrow it: every word must appear in a subject or body, and a filtered miss says so rather than reading as an empty inbox (`narrowed_by` in `--json`) |
...
cargo test                       # all 744 tests (746 on Linux)
...
| Read | For |
|---|---|
| [`docs/DECISIONS.md`](docs/DECISIONS.md) | **The specification.** D1–D141, each recording what was rejected and why |
